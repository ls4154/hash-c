...
static void _sha224_iterate(struct sha224_ctx *ctx, byte_t *chunk)
{
	word_t w[64];
...
void sha224_offline(struct sha224_ctx *ctx, byte_t *msg, len_t len)
{
	...
	while (len >= 64) {
		sha224_update(ctx, msg);

		msg += 64;
		len -= 64;
	}

	sha224_update_last(ctx, msg, len);
}
